...
### Added
- Added `setRootPath` and `setPort` for high level manipulation in class instance properties
- Added stricter checks for fields methods need with friendly error messages
- Added Mappings syncing and corrupted mappings handling

### Changed
- Made `upload` method options parameter optional

### Fixed
- File resource type resolving error
...
### Added
- Added TypeScript type declarations (`index.d.ts`) for full TypeScript support
- Added `"types"` field to `package.json` pointing to type definitions
...
- Fixed ES module import path in `index.js` by adding `.js` extension to `offline-cloudinary` import for proper CommonJS interoperability
